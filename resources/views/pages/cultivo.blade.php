...
@include('inc/checkdatacultivo')
@include('inc/savecultivo')
...
<?php
	...
	if ($_SERVER['REQUEST_METHOD']=="POST" && sizeof(session('error'))==0  && (!session('visitante'))){
		$saveworked = savecultivo($_POST,session('email'));
		if($saveworked=="true"){
...
<div class='bodycontainer' id="main">
    <section class="four">
        <div class="container">
            <header><h2>Monitoreo Productivo</h2></header>
                <div class=" warnings">
                    <?php
                        $hintlist = [
