...
function savenewentry(
    ...
    try {
        ...
        $sql3 = ") VALUES (";
        $sql5 = ");";
        foreach ($myarray as $column => $myval) {
            $columnarray .= $column . ',';
            if ($myval === "CURRENT_DATE") {
                $placeholder .= 'CURRENT_DATE,';
            } else {
                $placeholdername = ':value' . $column;
        ...
        //si el id de la tabla en BD no se asigna automáticamente, se calcula manualmente
        if ($manualID_inDB) {
...
function savenewentries(
    ...
    try {
        ...
        $results = DB::insert($completesql, $arraytopass);
        return "{$mytable} ha sido guardado con éxito";
    } catch (PDOException $e) {
        Log::info('savenewentry_fail:', [
            $e->getMessage(),
            'email' => $useremail,
        ...
        return "{$mytable} error al almacenar con el siguiente error: " . $e->getMessage();
    }
}
...
function savenewspecies($table, $comun, $cientifico, $invasor, $extra = false)
{
    ...
    $namesmatching = DB::select(
        "SELECT cientifico FROM {$table} WHERE cientifico=:value",
        [':value' => $cientifico]
    );
    ...
    try {
        ...
        foreach ($result as $colobj) {
            ...
            if (substr($col, 0, 4) != 'iden') {
                if ($withval) {
                    $colarray[$col] = trim(
                        $newpost["{$rowandnum}*{$tablename}*{$col}"]
                    );
                    //Check for incorrect datetime formats
                    if (
                        strpos($col, 'hora') !== false &&
                        strpos($colarray[$col], ':') == false
                    ) {
                        $colarray[$col] = "00:01";
                    }
                    if (
                        strpos($col, 'fecha') !== false &&
                        strpos($colarray[$col], '-') == false
                    ) {
...
function formatdate($locvalue, $sheet, $letter, $row_number)
{
    try {
        ...
        if (
            is_numeric(substr($locvalue, 3, 1)) &&
            !is_numeric(substr($locvalue, 4, 1))
        ) {
        ...
        if (ctype_alpha(substr($locvalue, 3, 3))) {
            if (
                strpos($rawmonth, 'ene') !== false ||
                strpos($rawmonth, 'jan') !== false
            ) {
                $rawmonth = '01';
                ...
                strpos($rawmonth, 'abr') !== false ||
                strpos($rawmonth, 'apr') !== false
            ) {
                $rawmonth = '04';
                ...
                strpos($rawmonth, 'aug') !== false ||
                strpos($rawmonth, 'ago') !== false
            ) {
                $rawmonth = '08';
                ...
                strpos($rawmonth, 'dic') !== false ||
                strpos($rawmonth, 'dec') !== false
            ) {
                $rawmonth = '12';
            }
        }
        $newloc =
            $rawmonth .
            "-" .
            explode($divider, $locvalue)[0] .
            "-" .
            explode($divider, $locvalue)[2];

        if (
            !(
                is_numeric(substr($newloc, 0, 2)) &&
                is_numeric(substr($newloc, 3, 2)) &&
                is_numeric(substr($newloc, 6, 4))
            )
        ) {
            return [
                $locvalue,
                "La fecha en {$letter}{$row_number} en {$sheet} es en formato incorrecto",
            ];
        }
        ...
        if (
            !(
                is_numeric(substr($newloc, 3, 2)) &&
                intval(substr($newloc, 3, 2)) > 0 &&
                intval(substr($newloc, 3, 2)) <= 31 &&
                is_numeric(substr($newloc, 0, 2)) &&
                intval(substr($newloc, 0, 2)) > 0 &&
                intval(substr($newloc, 0, 2)) < 13 &&
                is_numeric(substr($newloc, 6, 4)) &&
                intval(substr($newloc, 6, 4)) > 1899 &&
                intval(substr($newloc, 6, 4)) < 3000
            )
        ) {
            return [
                $newloc,
                "La fecha en {$letter}{$row_number} en {$sheet} tiene numeros incorrectos.",
            ];
        }
        if (
            !checkdate(
            ...
            return [
                $locvalue,
                "La fecha en {$letter}{$row_number} en {$sheet} no aparece en el calendario",
            ];
        }
        return [$newloc, ''];
    } catch (Exception $e) {
...
function formathour($locvalue, $sheet, $letter, $row_number)
{
    try {
        ...
        if (strlen($locvalue) < 3) {
            return [
                $locvalue,
                "La hora en {$letter}{$row_number} en {$sheet} es en formato incorrecto.",
            ];
        }
        ...
        if (
            !(
                is_numeric(substr($locvalue, 0, 2)) &&
                is_numeric(substr($locvalue, 3, 2)) &&
                substr($locvalue, 2, 1) == ':'
            )
        ) {
            return [
        ...
        if (
            !(
                is_numeric(substr($locvalue, 0, 2)) &&
                intval(substr($locvalue, 0, 2)) >= 0 &&
                intval(substr($locvalue, 0, 2)) < 24 &&
                is_numeric(substr($locvalue, 3, 2)) &&
                intval(substr($locvalue, 3, 2)) >= 0 &&
                intval(substr($locvalue, 3, 2)) < 60
            )
        ) {
            return [
                $locvalue,
        ...
        return [substr($locvalue, 0, 2) . ":" . substr($locvalue, 3, 2), ''];
    } catch (Exception $e) {
        return [
            $locvalue,
            "La hora en {$letter}{$row_number} en {$sheet} es en formato incorrecto.",
...
function uploadshape($shpname)
{
    if (strlen($_FILES[$shpname]["name"])) {
        ...
        if ($uploadOk == 0) {
            ...
            if (
                move_uploaded_file($_FILES[$shpname]["tmp_name"], $target_file)
            ) {
                echo "The file  has been uploaded.";
