...
function savedata($newpost, $useremail, $fromexcel = false)
{
    ...
    if (
        ...
        sizeof(session('error')) == 0 &&
        !session('visitante')
    ){
        if (
            isset($newpost['table']) &&
            ($newpost['select' . $newpost['table']] != 'Nuevo' ||
                strpos($newpost['table'], 'especie') !== false)
        ) {
            $table = $newpost['table'];
            ...
            foreach ($rowarray as $row => $keyandvalue) {
                ...
                if (strpos($table, 'especie') !== false) {
                    ...
                    if ($selection == 'Nuevo') {
                        savenewspecies(
                            $table,
                            $newpost["row0*{$table}*comun"],
                            $newpost["row0*{$table}*cientifico"],
                            $newpost["row0*{$table}*invasor"]
                        );
                        return true;
                    }
                ...
                if ($table == 'linea_mtp') {
                    ...
                    $updatedgeom = DB::update($updatesql, [
                        'selectedvalue' => $new_iden_nombre,
                    ]);
                    return true;
                }
            }
            ...
            if ($mtpchoice == "Nuevo") {
                ...
                if ($municipiochoice == "Nuevo") {
                    if ($estadochoice == "Nuevo") {
                        ...
                        $municipiofkey = askforkey(
                            "estado",
                            "clave",
                            "nombre",
                            $newpost['selectestado']
                        );
                    }
                    $municipiocolumns = [
                ...
                if ($newpost['selectpredio'] == "Nuevo") {
                    $prediofkey = askforkey(
                        "municipio",
                        "gid",
                        "nombre",
                        $newpost['selectmunicipio']
                    );
                    $prediocolumns = [
                        "nombre" => $newpost['row0*predio*nombre'],
                        "nombre_de_duenio_o_technico" =>
                            $newpost['row0*predio*nombre_de_duenio_o_technico'],
                        "referencia_de_accesso" =>
                            $newpost['row0*predio*referencia_de_accesso'],
                        "superficie" => $newpost['row0*predio*superficie'],
                        "contacto" => $newpost['row0*predio*contacto'],
                        "iden_muni_predio" =>
                            $newpost['selectmunicipio'] .
                            '-' .
                            $newpost['row0*predio*nombre'],
                ...
                if ($newpost['selectpredio'] == "Nuevo") {
                    ...
                    $linea_mtpfkey = askforkey(
                        ...
                        "iden",
                        "iden_muni_predio",
                        $newpost['selectpredio']
                    );
                    $linea_mtppredioname = $newpost['selectpredio'];
                }
                //Save New Linea_MTP Data
                for ($i = 0; $i < countrows($newpost, "linea_mtp"); $i++) {
                    $comienzo_latitud = round(
                        floatval(
                            $newpost["row{$i}*linea_mtp*comienzo_latitud"]
                        ),
                        6
                    );
                    $comienzo_longitud = round(
                        floatval(
                            $newpost["row{$i}*linea_mtp*comienzo_longitud"]
                        ),
                        6
                    );
                    $punto_2_latitud = round(
                        floatval($newpost["row{$i}*linea_mtp*punto_2_latitud"]),
                        6
                    );
                    $punto_2_longitud = round(
                        floatval(
                            $newpost["row{$i}*linea_mtp*punto_2_longitud"]
                        ),
                        6
                    );
                    $punto_3_latitud = round(
                        floatval($newpost["row{$i}*linea_mtp*punto_3_latitud"]),
                        6
                    );
                    $punto_3_longitud = round(
                        floatval(
                            $newpost["row{$i}*linea_mtp*punto_3_longitud"]
                        ),
                        6
                    );
                    $punto_4_latitud = round(
                        floatval($newpost["row{$i}*linea_mtp*punto_4_latitud"]),
                        6
                    );
                    $punto_4_longitud = round(
                        floatval(
                            $newpost["row{$i}*linea_mtp*punto_4_longitud"]
                        ),
                        6
                    );
                    $fin_latitud = round(
                        floatval($newpost["row{$i}*linea_mtp*fin_latitud"]),
                        6
                    );
                    $fin_longitud = round(
                        floatval($newpost["row{$i}*linea_mtp*fin_longitud"]),
                        6
                    );
                ...
                if ($medicionchoice == "Nuevo") {
                    ...
                    $linea_mtpclave_predio = askforkey(
                        "linea_mtp",
                        "iden_predio",
                        "nombre_iden",
                        $mtpchoice
                    );
                    $predioname = askforkey(
                        "predio",
                        "nombre",
                        "iden",
                        $linea_mtpclave_predio
                    );
                    
                    $spanishdate =date('d-m-Y', strtotime(str_replace('-','/',$newpost['row0*medicion*fecha'])));
                    if (!$fromexcel) {
                        $spanishdate =date('d-m-Y', strtotime(str_replace('-','/',$newpost['row0*medicion*fecha'])));
                    }
                    $medicioncolumns = [
                    ...
                    for ($i = 0; $i < rowmax($newpost, "personas"); $i++) {
                        if (isset($newpost["row{$i}*personas*nombre"])) {
                            $personascolumns = [
                                ...
                                "apellido_materno" =>
                                    $newpost[
                                        "row{$i}*personas*apellido_materno"
                                    ],
                                "apellido_paterno" =>
                                    $newpost[
                                        "row{$i}*personas*apellido_paterno"
                                    ],
                            ];
                            $resultofquery[] = savenewentry(
                                "personas",
                    ...
                    for ($i = 0; $i < rowmax($newpost, "gps"); $i++) {
                        if (isset($newpost["row{$i}*gps*anio"])) {
                            $gpscolumns = [
                                ...
                                "numero_de_serie" =>
                                    $newpost["row{$i}*gps*numero_de_serie"],
                            ];
                    ...
                    for ($i = 0; $i < rowmax($newpost, "camara"); $i++) {
                        if (isset($newpost["row{$i}*camara*anio"])) {
                            $camaracolumns = [
                                ...
                                "numero_de_serie" =>
                                    $newpost["row{$i}*camara*numero_de_serie"],
                            ];
                            $resultofquery[] = savenewentry(
                                "camara",
                                $camaracolumns
                    ...
                    if ($newpost['mode'] == 'Datos Existentes') {
                        ...
                        $transpunto = 'punto';
                        if (
                            $obstype == "observacion_hierba" ||
                            $obstype == "observacion_herpetofauna"
                        ) {
                        ...
                        $numrows = DB::delete($sql, [
                            'iden_email' => str_replace('"', '', $useremail),
                        ]);
                    ...
                    $unitcolumns = buildcolumnsarray(
                        $newpost,
                        "{$transpunto}_{$speciestype}",
                        "row0"
                    );
                    $unitcolumns["iden_sampling_unit"] = $unitnum;
                    if($lifeform=='mamifero'&&$fromexcel){
                        $unitcolumns['fecha_de_activacion']=date('d-m-Y', strtotime(str_replace('-','/',$unitcolumns['fecha_de_activacion'])));
                        $unitcolumns['fecha_de_apagado']=date('d-m-Y', strtotime(str_replace('-','/',$unitcolumns['fecha_de_apagado'])));
                    }
                    ...
                    for ($i = 0; $i < countrows($newpost, $obstype); $i++) {
                        ...
                        if ($fromexcel) {
                            if($obstype=='observacion_mamifero'){
                                $newpost["row{$i}*{$obstype}*fecha_de_captura"]=date('d-m-Y', strtotime(str_replace('-','/',$newpost["row{$i}*{$obstype}*fecha_de_captura"])));
                            }
                            ...
                            $iden_foto = uploadfoto(
                                $newpost,
                                $_FILES[$postid]["name"],
                                $_FILES[$postid]["tmp_name"],
                                $_FILES[$postid]["size"],
                                $obstype
                            );
                        }
                        ...
                        if ($especiechoice == "Nuevo") {
                            $iden_especie = savenewspecies(
                                $speciestable,
                                $newpost["row{$i}*{$obstype}*comun"],
                                $newpost["row{$i}*{$obstype}*cientifico"],
                                isset($newpost["row{$i}*{$obstype}*invasor"])
                            );
                        } else {
                            $iden_especie = askforkey(
                                $speciestable,
                                "iden",
                                "cientifico",
                                $especiechoice
                        ...
                        if (
                            $iden_foto == 'No Presentado' ||
                            explode("_", $iden_foto)[0] == 'observacion'
                        ) {
                            $numero = 1;
                            if (
                                isset(
                                    $newpost["row{$i}*{$obstype}*cantidad"]
                                ) &&
                                intval(
                                    $newpost["row{$i}*{$obstype}*cantidad"]
                                ) > 0
                    ...
                    if (sizeof($resultofquery) == 0) {
                        ...
                        //SAVE unidad de analisis a la que pertenece el punto de observación
                        //se obtienen las unidades de análisis (uda) que se intersecan
                        //con el punto de la línea TIM que se está analizando
                        $sql = "SELECT ushp.gid FROM usershapes ushp WHERE ST_Intersects(ushp.geom, ST_GeomFromText('POINT({$mylong} {$mylat})',4326))";
                        $udasXPuntos = DB::select($sql, []);
                        $tmpSpeciesType=$speciestype;//uso mi propia variable temporal, porque el código está horribe y no sé en donde más usen la variable $speciestype de nuevo, asi que mejor la dejo como está.
                        if($speciestype=='herpetofauna'){
                            $tmpSpeciesType='herpeto';
                        }
                        $sql = "SELECT nombre_min FROM especies WHERE nombre ='{$tmpSpeciesType}'";
                        $especie = DB::select($sql, [])[0]->nombre_min;
                        if (sizeof($udasXPuntos) > 0) {
                            foreach ($udasXPuntos as $udaXPunto) {
                                //Se obtiene el id del último puntoMTP agregado y se relaciona
                                //con todos los shapes con los que se interseca
                                $cols = [];
                                $cols['iden_punto'] = $unitmax;
                                $cols['iden_medicion'] =
                                    $unitcolumns["iden_medicion"];
                                $cols['iden_uda'] = $udaXPunto->gid;
                                $cols['especie'] = $especie;
                                //se guarda la relación uda <-> punto TIM
                                $resultUdaXPuntoInsert = savenewentry(
                                    "puntosmtp_udas",
                                    $cols,
                                    false,
                                    false
